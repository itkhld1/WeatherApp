...
class WeatherManager{
    func getCurrentWeather(latitute: CLLocationDegrees, longitude: CLLocationDegrees) async throws -> ResponseBody {
        guard let url = URL(string: "https://api.openweathermap.org/data/2.5/weather?lat=\(latitute)&lon=\(longitude)&appid=\("abcde")&units=metric") else {fatalError("Missing URL")}
